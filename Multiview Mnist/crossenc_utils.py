...
def create_dataset(rng):
    ...
    mat = scipy.io.loadmat('MNIST_dataset.mat')
    X = np.double(mat['X'])
    XTe = np.double(mat['X_Te'])
    label = np.double(mat['labels']).ravel().astype(int)
    labelTe = np.double(mat['labels_Te']).ravel().astype(int)

    idx = rng.permutation(X.shape[0])
    XV = X[idx[50000:], :]
    X = X[idx[:50000], :]
    labelV = label[idx[50000:]]
    label = label[idx[:50000]]

    ind_tr, ind_v, ind_te = [], [], []
    for i in range(10):
        ind = np.where(label == i)
        ind_tr.append(rng.permutation(ind[0]))
        ind = np.where(labelV == i)
        ind_v.append(rng.permutation(ind[0]))
        ind = np.where(labelTe == i)
        ind_te.append(rng.permutation(ind[0]))
    ...
    X1 = X + rng.uniform(0, 1, (50000, 28 * 28))
    X1 = truncate(X1)
    X2 = np.zeros((np.shape(X1)))
    ind_x = np.zeros(10).astype(int)
    for i in range(50000):
        ind = ind_tr[label[i]]
        ind = ind[ind_x[label[i]]]
        ind_x[label[i]] = ind_x[label[i]] + 1
        b = copy.deepcopy(X[ind])
        b = ndimage.rotate(b.reshape(28, 28), 90 * rng.uniform(0, 1) - 45, reshape=False)
        X2[i] = truncate(b.reshape(1, 28 * 28))
        # Z = np.concatenate((X[i].reshape(28, 28), X1[i].reshape(28, 28), X2[i].reshape(28, 28), X[ind].reshape(28, 28)), axis=1)
        # plt.matplotlib.pyplot.imshow(Z, cmap=plt.cm.gray)
        # plt.show()

    # Normalize
    for i in range(10000):
        XV[i] = normalize(XV[i])
        XTe[i] = normalize(XTe[i])

    XV1 = XV + rng.uniform(0, 1, (10000, 28 * 28))
    XTe1 = XTe + rng.uniform(0, 1, (10000, 28 * 28))
    XV1, XTe1 = truncate(XV1), truncate(XTe1)
    XV2, XTe2 = np.zeros((np.shape(XV1))), np.zeros((np.shape(XTe1)))
    ind_x1, ind_x2 = np.zeros(10).astype(int), np.zeros(10).astype(int)
    for i in range(10000):
        ind = ind_v[labelV[i]]
        ind = ind[ind_x1[labelV[i]]]
        ind_x1[labelV[i]] = ind_x1[labelV[i]] + 1
        b = copy.deepcopy(XV[ind])
        b = ndimage.rotate(b.reshape(28, 28), 90 * rng.uniform(0, 1) - 45, reshape=False)
        XV2[i, :] = truncate(b.reshape(1, 28 * 28))
        ind = ind_te[labelTe[i]]
        ind = ind[ind_x2[labelTe[i]]]
        ind_x2[labelTe[i]] = ind_x2[labelTe[i]] + 1
        b = copy.deepcopy(XTe[ind])
        b = ndimage.rotate(b.reshape(28, 28), 90 * rng.uniform(0, 1) - 45, reshape=False)
        XTe2[i, :] = truncate(b.reshape(1, 28 * 28))
        # Z = np.concatenate(
        #    (XTe[i].reshape(28, 28), XTe1[i].reshape(28, 28), XTe2[i].reshape(28, 28), b.reshape(28, 28)), axis=1)
        # plt.matplotlib.pyplot.imshow(Z, cmap=plt.cm.gray)
        # plt.show()

    return X1, X2, XV1, XV2, XTe1, XTe2, label, labelTe
...
def train_Lyu_et_all(i_shape, args, num_init, data_tr, data_val, data_te):
    ...
    for lambda_ in p_l:
        ...
        for i in range(num_init):
            if True:
                # Initiate the models
                ae_model = l_model.CNNDAE(args.o_dim, args.o_dim2, channels=1).to(device)
                ...
                if args._lambda > 0:
                    ...
                    mmcca1 = []
                    mmcca2 = []
                    optimizer = torch.optim.AdamW(ae_model.parameters(), lr=args.lr_min, weight_decay=args.weight_decay, amsgrad=True)
            ...
            for itr in range(1, args.num_iters + 1):
                ...
                for _ in range(args.inner_epochs):
                    ...
                    # Save the model
                    if (
                            1 - args.beta) * match_err_val + args.beta * recons_err_val + args._lambda * corr_val < best_obj_val:
                        print('Saving Model')
                        best_ae_model = ae_model
                        best_obj_tr = (
                                              1 - args.beta) * match_err_tr + args.beta * recons_err_tr + args._lambda * corr_tr
                        best_obj_val = (
                                               1 - args.beta) * match_err_val + args.beta * recons_err_val + args._lambda * corr_val
...
class CnnEncModel(nn.Module):
    def __init__(self, i_shape, mid_shape, mid_shape2, o_dim, p):
        super(CnnEncModel, self).__init__()
...
class CnnDecModel(nn.Module):
    def __init__(self, i_shape, mid_shape, mid_shape2, o_dim, p):
        super(CnnDecModel, self).__init__()
        self.i_shape = i_shape
        self.mid_shape = mid_shape
        self.mid_shape2 = mid_shape2
        self.o_dim = o_dim
...
class CEModel(nn.Module):
    def __init__(self, i_shape, o_dim, dataset, p=0):
        ...
        if dataset == 1:
            self.enc_net0 = CnnEncModel(self.i_shape[0], self.mid_shape, self.mid_shape2, self.o_dim, self.p)
            self.enc_net1 = CnnEncModel(self.i_shape[1], self.mid_shape, self.mid_shape2, self.o_dim, self.p)
            self.dec_net0 = CnnDecModel(self.i_shape[0], self.mid_shape, self.mid_shape2, self.o_dim, self.p)
            self.dec_net1 = CnnDecModel(self.i_shape[1], self.mid_shape, self.mid_shape2, self.o_dim, self.p)

...
def load_phoneme_dataset():
    # Load the dataset
    mat = scipy.io.loadmat('XRMBf2KALDI_window7_single1.mat')
    X1 = mat['X1']
    XV1 = mat['XV1']
    XTe1 = mat['XTe1']

    mat = scipy.io.loadmat('XRMBf2KALDI_window7_single2.mat')
    X2 = mat['X2']
    XV2 = mat['XV2']
    XTe2 = mat['XTe2']
    ...
    X3 = mat['TrLabel']
    XV3 = mat['TuLabel']
    XTe3 = mat['TeLabel'] @ np.arange(40)

    return X1, X2, XV1, XV2, XTe1, XTe2, XTe3


def load_reduced_phoneme_dataset():
    # Load the dataset
    mat = scipy.io.loadmat('XRMB_reduced.mat')
    X1 = mat['X1']
    XV1 = mat['XV1']
    XTe1 = mat['XTe1']
    X2 = mat['X2']
    ...
    XV3 = mat['XV3']
    XTe3 = mat['XTe3']
    label = mat['trainLabel'] + 1
    label = label.ravel()
    labelTe = mat['testLabel'] + 1
    labelTe = labelTe.ravel()

    return X1, X2, X3, XV1, XV2, XV3, XTe1, XTe2, XTe3, label, labelTe
...
def train_Lyu_et_all(i_shape, args, num_init, data_tr, data_val, data_te):
    ...
    for lambda_ in p_l:
        ...
        for i in range(num_init):
            if True:
                # Initiate the models
                ae_model = l_model.DAE(args.o_dim, args.o_dim2, i_shape).to(device)
                ...
                if args._lambda > 0:
                    ...
                    mmcca2 = l_model.MMDCCA(args.o_dim, args.o_dim2[1], [args.phi_hidden_size] * args.phi_num_layers,
                                            [args.tau_hidden_size] * args.tau_num_layers).to(device)
                    # View3 independence regularization network
                    # mmcca3 = l_model.MMDCCA(args.o_dim, args.o_dim2[2], [args.phi_hidden_size] * args.phi_num_layers,
                    #                      [args.tau_hidden_size] * args.tau_num_layers).to(device)
                    mmcca3 = []
                    ...
                    optimizer = torch.optim.AdamW([
                        ...
                        # {'params': mmcca3.parameters(), 'lr': args.lr_max},
                ...
                view2 = data_tr[:, i_shape[0]:i_shape[0]+i_shape[1]]
                view3 = data_tr[:, i_shape[0] + i_shape[1]:]
                norm_views = [torch.linalg.norm(view1, 'fro') ** 2, torch.linalg.norm(view2, 'fro') ** 2, torch.linalg.norm(view3, 'fro') ** 2]

                view1_val = data_val[:, :i_shape[0]]
                view2_val = data_val[:, i_shape[0]:i_shape[0] + i_shape[1]]
                view3_val = data_val[:, i_shape[0] + i_shape[1]:]
                norm_views_val = [torch.linalg.norm(view1_val, 'fro') ** 2, torch.linalg.norm(view2_val, 'fro') ** 2, torch.linalg.norm(view3_val, 'fro') ** 2]

                train_loader_b1 = l_utils.get_dataloader(view1, view2, view3, args.batchsize1, True)
                eval_loader = l_utils.get_dataloader(view1, view2, view3, args.batchsize2, False)
                eval_loader_val = l_utils.get_dataloader(view1_val, view2_val, view3_val, args.batchsize2, False)
                train_loader_b2 = l_utils.get_dataloader(view1, view2, view3, args.batchsize2, True)
            ...
            for itr in range(1, args.num_iters + 1):
                ...
                for _ in range(args.inner_epochs):
                    ...
                    # Backprop to update
                    corr_iter = l_train.train(ae_model, mmcca1, mmcca2, mmcca3, U, norm_views, train_loader_b1, train_loader_b2,
                                              corr_iter, args, optimizer, device)

                    # Evaluate on the whole training set
                    match_err_tr, recons_err_tr, corr_tr = l_train.eval_train(ae_model, mmcca1, mmcca2, mmcca3, itr, U,
                                                                              norm_views, eval_loader, args, device)

                    # Evaluate on the whole validation set
                    match_err_val, recons_err_val, corr_val = l_train.eval_val(ae_model, mmcca1, mmcca2, mmcca3, itr,
...
def get_loss2(A, model, i_shape, batch_size, lambda_):
    ...
    lambda_1 = (1 - lambda_)
    lambda_2 = lambda_
    loss = lambda_1 * mse1 + lambda_2 * mse2
    return loss
...
def train(data_tr, data_val, data_te, model, model_optimizer, batch_size, epoch_num, i_shape, lambda_):
    ...
    for epoch in range(epoch_num):  # loop over the dataset multiple times
        ...
        with torch.no_grad():
            ...
            val_loss = get_loss2(data_val, model, i_shape, batch_size, lambda_)
            te_loss = get_loss2(data_te, model, i_shape, batch_size, lambda_)
            ...
            if (val_loss < min_loss) & (epoch - last > 10): #10
                best_model = model
                best_epoch = epoch
                min_loss = val_loss
...
def compute_MAXVAR(X, XV, XTe, o_dim):

    U = []
    S = []
    Vh = []
    for i in range(len(X)):
        mean_tmp = torch.mean(X[i], 0)
        X[i] = X[i] - mean_tmp
        U_tmp, S_tmp, V_tmp = torch.linalg.svd(X[i], full_matrices=False)
        U.append(U_tmp)
        S.append(S_tmp)
        Vh.append(V_tmp)

    G, Lambda, _ = torch.linalg.svd(torch.cat((U), 1), full_matrices=False)
    G = G[:, :o_dim]
    Lambda = Lambda[:o_dim]**2

    Q = []
    for i in range(len(X)):
        smat = torch.diag(S[i]**(-1))
        Q_tmp = (G.T @ U[i]) @ (smat @ Vh[i])
        Q.append(Q_tmp.T)
    ...
    EncTe = []
    for i in range(len(X)):
        mean_tmp = torch.mean(XV[i], 0)
        XV[i] = XV[i] - mean_tmp
        mean_tmp = torch.mean(XTe[i], 0)
        XTe[i] = XTe[i] - mean_tmp
        A = (X[i] @ Q[i]).detach().cpu().numpy()
        B = (XV[i] @ Q[i]).detach().cpu().numpy()
        C = (XTe[i] @ Q[i]).detach().cpu().numpy()
        EncTr.append(A)
        EncV.append(B)
        EncTe.append(C)

    linear_cca_corr = np.zeros(3)
    corr_tr1, _ = my_corr(EncTr[0], EncTr[1], o_dim)
    corr_tr2, _ = my_corr(EncTr[0], EncTr[2], o_dim)
    corr_tr3, _ = my_corr(EncTr[2], EncTr[1], o_dim)
    linear_cca_corr[0] = (1/3) * (corr_tr1 + corr_tr2 + corr_tr3)

    corr_val1, _ = my_corr(EncV[0], EncV[1], o_dim)
    corr_val2, _ = my_corr(EncV[0], EncV[2], o_dim)
    corr_val3, _ = my_corr(EncV[2], EncV[1], o_dim)
    linear_cca_corr[1] = (1/3) * (corr_val1 + corr_val2 + corr_val3)

    corr_te1, _ = my_corr(EncTe[0], EncTe[1], o_dim)
    corr_te2, _ = my_corr(EncTe[0], EncTe[2], o_dim)
    corr_te3, _ = my_corr(EncTe[2], EncTe[1], o_dim)
    linear_cca_corr[2] = (1 / 3) * (corr_te1 + corr_te2 + corr_te3)

    MAXVAR_enc = [0.5 * (EncTr[0] + EncTr[1]), 0.5 * (EncV[0] + EncV[1]), 0.5 * (EncTe[0] + EncTe[1])]

    return linear_cca_corr, Q, MAXVAR_enc, Lambda
...
def compute_corr_coef_DGCCA(data_tr, data_val, data_te, model, o_dim):
    ...
    with torch.no_grad():
        ...
        for k in range(3):
            ...
            join_enc.append(0.5 * (pred_out[0][0]+pred_out[0][1]))
            del temp, pred_out

        W1 = [new_data[0][0], new_data[1][0], new_data[2][0]]
        W2 = [new_data[0][1], new_data[1][1], new_data[2][1]]
        W3 = [new_data[0][2], new_data[1][2], new_data[2][2]]

        # Compute the correlation coefficients
        corr_l1, D_l1 = lcca_method(W1, W2, o_dim)
        corr_l2, D_l2 = lcca_method(W1, W3, o_dim)
        corr_l3, D_l3 = lcca_method(W2, W3, o_dim)
        corr_te = (1 / 3) * (corr_l1[2] + corr_l2[2] + corr_l3[2])
        Ds = [D_l1, D_l2, D_l3]

    return corr_te, Ds, join_enc
    ...
    for s in range(num_of_inits):
        ...
        with torch.no_grad():
            ...
            for k in range(3):
                ...
                pred_out = model_cp(temp)
                new_data.append(pred_out[0])
                join_encs.append(0.5 * (pred_out[0][0]+pred_out[0][1]))
                del temp, pred_out
            ...
            corr_l2, D_l2 = lcca_method(W1, W3, o_dim)
            corr_l3, D_l3 = lcca_method(W2, W3, o_dim)
            corr_te.append((1/3) * (corr_l1[2]+corr_l2[2]+corr_l3[2]))
            Ds.append([D_l1, D_l2, D_l3])
...
def compute_corr_coef_over_lambda(data_tr, data_val, data_te, model, o_dim):
    ...
    for l in range(num_of_models):
        ...
        with torch.no_grad():
            ...
            corr_l2, D_l2 = lcca_method(W1, W3, o_dim)
            corr_l3, D_l3 = lcca_method(W2, W3, o_dim)
            corrs_te.append((1 / 3) * (corr_l1[2] + corr_l2[2] + corr_l3[2]))
            Ds.append([D_l1, D_l2, D_l3])
    ...
    for l in range(num_of_lambdas):
        ...
        for s in range(num_of_inits):
            ...
            with torch.no_grad():
                ...
                corr_l2, D_l2 = lcca_method(W1, W3, o_dim)
                corr_l3, D_l3 = lcca_method(W2, W3, o_dim)
                corrs_over_init.append((1 / 3) * (corr_l1[2] + corr_l2[2] + corr_l3[2]))
                Ds_over_init.append([D_l1, D_l2, D_l3])
                join_encs_over_init.append(join_encs)
    ...
    for l in range(num_of_models):
        ...
        with torch.no_grad():
            ...
            for k in range(3):
                ...
                view2 = temp[:, i_shape[0]:i_shape[0] + i_shape[1]]
                view3 = temp[:, i_shape[0] + i_shape[1]:]
                pred_out = model_cp.encode([view1, view2, view3])
                new_data.append(pred_out[0])
                join_enc.append(0.5 * (pred_out[0][0]+pred_out[0][1]))
                del temp, pred_out
